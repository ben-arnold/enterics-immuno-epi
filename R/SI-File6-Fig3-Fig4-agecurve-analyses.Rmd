...
subtitle: "Supporting Information File 6: Estimation of age-dependent means and seroprevalence using multiple approaches, Figures 3 and 4"
output: 
  html_notebook:
    theme: default
    highlight: haddock
...
# Notebook Summary
This notebook estimates age-dependent mean antibody and seroprevalence curves among children from Haiti, Kenya, and Tanzania. Estimators include cubic splines (in a GAM model from the `mgcv` package) and  ensemble machine learning (using stacked super learning from the `tmleAb` package).  Spline-based estimates are reported in the main text in Figures 3 and 4. 
...
ggsave(here("figs","Fig3-haiti-ab-agecurves-mean.pdf"),plot=pcurve,device="pdf",width=7,height=14)
ggsave(here("figs","Fig3-haiti-ab-agecurves-mean.TIFF"),plot=pcurve,device="tiff",width=7,height=14)
...
## Figure 4,  Age-dependent seroprevalence curves
Create a more parsimonius plot, that only includes the spline fits but also includes the actual points.

** This is Figure 4 in the text **

```{r age-curve seroprevalence figure, fig.height=9, fig.width=6}

# re-arrange antigen factor to group noro on the same row
dplot <- dplot %>%
  mutate(antigenf=factor(antigenf,levels=c("Giardia VSP-3 or VSP-5","Cryptosporidium Cp17 or Cp23","E. histolytica LecA","Salmonella LPS groups B or D","Norovirus GI","Norovirus GII","ETEC toxin beta subunit")))

# bright color-blind palette defined in an earlier code chunk
pcols <- c(cred,corange,cgreen,cteal,cmagent,cmagent,cblue)
...
# save PDF and TIFF versions
ggsave(here("figs","Fig4-haiti-ab-agecurves-seroprev.pdf"),plot=ppcurve2,device="pdf",width=7,height=14)
ggsave(here("figs","Fig4-haiti-ab-agecurves-seroprev.TIFF"),plot=ppcurve2,device="tiff",width=7,height=14)
...
dplot <- gamfits %>%
  filter(antigenf!="Cholera toxin beta subunit") %>%
  mutate(antigenf=factor(antigenf,levels=c("Giardia VSP-3","Giardia VSP-5",
                                           "Cryptosporidium Cp17","Cryptosporidium Cp23",
                                           "E. histolytica LecA","ETEC toxin beta subunit",
                                           "Salmonella LPS B","Salmonella LPS D",
                                           "Campylobacter p18","Campylobacter p39" )))

# bright color-blind palette defined in an earlier code chunk
pcols <- c(cred,corange,cteal,cgreen,cblue,cmagent)
...
# save PDF and TIFF versions
ggsave(here("figs","Fig3sup2-asembo-ab-agecurves-mean.pdf"),plot=pcurve,device="pdf",width=7,height=14)
ggsave(here("figs","Fig3sup2-asembo-ab-agecurves-mean.TIFF"),plot=pcurve,device="tiff",width=7,height=14)
```
...
```
## Figure 4-supplement 1,  Age-dependent seroprevalence curves

** This is Figure 4 - supplement 1in the text **
...
# save PDF and TIFF versions
ggsave(here("figs","Fig4sup1-asembo-ab-agecurves-seroprev.pdf"),plot=ppcurve2,device="pdf",width=7,height=10)
ggsave(here("figs","Fig4sup1-asembo-ab-agecurves-seroprev.TIFF"),plot=ppcurve2,device="tiff",width=7,height=10)
...
mbalabs <- c("Giardia VSP-3","Giardia VSP-5","Cryptosporidium Cp17","Cryptosporidium Cp23",
             "E. histolytica LecA","Salmonella LPS B","Salmonella LPS D","ETEC toxin beta subunit","Cholera toxin beta subunit","Campylobacter p18","Campylobacter p39")
...
# drop cholera, bump ETEC up to put salmonella on the same row
# this is only for final aesthetics
dplot <- gamfits %>%
  filter(antigenf!="Cholera toxin beta subunit") %>%
...
pcurve <- ggplot(data=dplot,aes(x=age+0.5,color=pathogen,fill=pathogen)) +
  ...
  geom_ribbon(aes(ymin=lwrS,ymax=uprS),alpha=0.2,color=NA,fill=cgrey) +
  # actual measurements
  geom_jitter(aes(y=logmfi),width=0.2,height=0,alpha=0.3,size=0.3) +
  # ensemble fit
  # geom_line(data=slfits,aes(x=age,y=fit),color=cgrey) +
  # spline fit
  geom_line(aes(y=fit),color="black") +
...
# save PDF and TIFF versions 
ggsave(here("figs","Fig3sup1-kongwa-ab-agecurves-mean.pdf"),plot=pcurve,device="pdf",width=7,height=14)
ggsave(here("figs","Fig3sup1-kongwa-ab-agecurves-mean.TIFF"),plot=pcurve,device="tiff",width=7,height=14)
...
dgi <- dlp %>% 
  filter(antigen %in% c("vsp3","vsp5")) %>%
  select(svy,year,vilid,clusterid,hhid,seroid,id,aztr,age,pathogen,antigen,seropos) %>%
  spread(antigen,seropos) %>%
  mutate(seropos=ifelse(vsp3==1|vsp5==1,1,0),
         antigen="vsp3vsp5",
         antigenf="Giardia VSP-3 or VSP-5") %>%
...
dlp <- bind_rows(dlp,dgi,dcr) %>%
  ...
  filter(!is.na(antigenf)) %>%
  select(pathogen,antigen,antigenf,
         svy,year,vilid,clusterid,hhid,seroid,id,aztr,age,
...
## Figure 4 - supplement 2, Age-dependent seroprevalence
...
** This is Figure 4 - supplement 2 in the manuscript **
...
# save a PDF version
ggsave(here("figs","Fig4sup2-kongwa-ab-agecurves-seroprev.pdf"),plot=ppcurve2,device="pdf",width=4,height=10)
ggsave(here("figs","Fig4sup2-kongwa-ab-agecurves-seroprev.TIFF"),plot=ppcurve2,device="tiff",width=4,height=10)

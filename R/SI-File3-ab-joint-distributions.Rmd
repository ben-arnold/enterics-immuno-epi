...
d <- dall %>%
  ...
  mutate(xlab=ifelse(antigen %in% c("vsp3","vsp5"),"VSP-3",NA),
         ...
         xlab=ifelse(antigen %in% c("cholera","etec"),"V. cholerae toxin beta subunit",xlab),
         ylab=ifelse(antigen %in% c("cholera","etec"),"ETEC toxin beta subunit",ylab),
         xlab=ifelse(antigen %in% c("norogi","norogii"),"GII",xlab),
         ylab=ifelse(antigen %in% c("norogi","norogii"),"GI",ylab)
         ) %>%
  filter(!is.na(comp))
...
ggsave(here("figs","Fig2-ab-scatter-composite.pdf"),plot=complot,device="pdf",width=13,height=15)

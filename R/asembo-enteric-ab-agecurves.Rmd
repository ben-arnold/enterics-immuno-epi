...
# save a PDF version 
ggsave("~/enterics-immuno-epi/figs/asembo-ab-agecurves-mean-allestimates.pdf",plot=pcurve,device="pdf",width=7,height=14)
...
ds <- dl %>% 
  ...
  mutate(seropos=ifelse(salb==1|sald==1,1,0),
         antigen="salbsald",
         antigenf="Salmonella LPS B or D") %>%
  select(-salb,-sald)

dlp <- bind_rows(dl,dgi,dcr,dcp,ds) %>%
...
ppcurve <- ggplot(data=pgamfits,aes(x=age,group=pathogen,color=pathogen,fill=pathogen)) +
  ...
  geom_line(aes(y=fit)) +
  # ensemble fit
  geom_line(data=pslfits,aes(x=age,y=fit),col=cgrey) +
  # empirical means 95% CI from the influence curve (averaged for each age)
...
# save a PDF version
ggsave("~/enterics-immuno-epi/figs/asembo-ab-agecurves-seroprev-allests.pdf",plot=ppcurve,device="pdf",width=7,height=14)

```
Create a more parsimonius figure restricted to spline fits. Plot through age 17 months (since n=1 at 18 months).
...
ppcurve <- ggplot(data=filter(pgamfits,age<=17),aes(x=age,group=pathogen,color=pathogen,fill=pathogen)) +
  # approximate simultaneous CI from spline fit
  geom_ribbon(aes(ymin=lwrS,ymax=uprS),alpha=0.2,color=NA,fill=cgrey) +
  # actual measurements
  geom_jitter(aes(y=seropos),width=0.2,height=0,color="black",alpha=0.4,size=0.3) +
...
Compressed figure (1 panel)

```{r age-curve seroprevalence figure small, fig.height=5, fig.width=5}

plabs <- pgamfits %>%
  filter(age==17) %>% select(antigenf,age,fit) %>% 
  group_by(antigenf) %>% summarize_all(mean) %>%
  mutate(fit=ifelse(antigenf=="Campylobacter p18 or p39",fit+0.02,fit),
         fit=ifelse(antigenf=="Cholera toxin beta subunit",fit-0.02,fit))

# bright color-blind palette defined in an earlier code chunk
pcols <- c(cred,corange,cgreen,cchartr,cteal,cmagent,cblue,cgrey)

ppcurve3 <- ggplot(data=filter(pgamfits,age<=17),aes(x=age,group=antigenf,color=antigenf,fill=antigenf)) +
  # approximate simultaneous CI from spline fit
  ...
  # plot aesthetics
  # facet_wrap(~antigenf,nrow=6,ncol=2) +
  scale_x_continuous(breaks=seq(0,16,by=2),limits=c(0,22))+
  scale_y_continuous(breaks=seq(0,1,by=0.2),labels=sprintf("%1.0f",seq(0,1,by=0.2)*100))+
  scale_fill_manual(values=pcols) +
  scale_color_manual(values=pcols,guide=guide_legend(title="")) +
  coord_cartesian(ylim=c(0,1))+
  labs(x="Age in months",y="Seroprevalence (%)") +
...
# save a PDF version
ggsave("~/enterics-immuno-epi/figs/asembo-ab-agecurves-seroprev-small.pdf",plot=ppcurve3,device="pdf",width=9,height=7)

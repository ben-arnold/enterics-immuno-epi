---
title: "Insights into the seroepidemiology of enteric pathogens among children living in high transmission settings"
subtitle: "Supplementary Information File 1"
output: 
  html_document:
    highlight: haddock
    theme: default
...
gamfits <- foreach(tri=levels(dl$tr),.combine=rbind) %:%
  foreach(ab=levels(dl$antigenf),.combine=rbind) %dopar% {
    ...
    gfit <- mgcv::gam(logmfi~s(age, bs="cr",k=9),data=pd,family="gaussian")
    gsci <- gamCI(m=gfit,newdata=pd,nreps=1000)
    gsci <- gsci %>% mutate(tr=tri,antigenf=ab)
    return(gsci)
  }

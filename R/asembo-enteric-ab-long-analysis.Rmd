...
dlpw <- dlp %>%
  select(childid,antigen,time,seropos) %>%
  spread(time,seropos) %>% 
...
dl2 <- dl2 %>% 
  mutate(seroposf=ifelse(seropos==1,"Seropositive","Seronegative"),
         poscut=ifelse(!is.na(posroc),log10(roccut),mixcut))

pdist <- ggplot(data=filter(dl2,seroi==1),aes(x=logmfi,group=seroposf,color=seroposf,fill=seroposf))+
  ...
  geom_vline(aes(xintercept=poscut))+

...
#-----------------------------
dl <- readRDS("~/enterics-immuno-epi/data/assembo_analysis.rds")
...
# based on seropositives identified through the mixture model
# subset to the individuals measured at both timepoints
dlpw <- dlp %>%
  select(childid,antigen,time,posmix) %>%
  spread(time,posmix) %>% 
...
# save a PDF version
ggsave("~/dropbox/asembo/results/figs/asembo-ab-long-mfi.pdf",plot=ppq,device="pdf",width=5,height=10)
...
# save a PDF version
ggsave("~/dropbox/asembo/results/figs/asembo-ab-long-seroincidence.pdf",plot=pp,device="pdf",width=5,height=8)
...
dl2 <- dl2 %>% mutate(posmixf=ifelse(posmix==1,"Seropositive","Seronegative"))
pdist <- ggplot(data=filter(dl2,seroi==1),aes(x=logmfi,group=posmixf,color=posmixf,fill=posmixf))+
  ...
  geom_vline(aes(xintercept=mixcut))+
...
# first measurement (dp$posmix==0)
#-----------------------------
est_seroi <- foreach(ab=levels(dp$antigenf),.combine=rbind) %do% {
  res <- exactprev(dp$seroi[dp$antigenf==ab & dp$posmix==0])
  res <- data.frame(antigenf=ab,N=res[1],n=res[2],mean=res[3],min95=res[4],max95=res[5])
  res
}
#-----------------------------
...
# first measurement (dp$posmix==1)
#-----------------------------
est_seror <- foreach(ab=levels(dp$antigenf),.combine=rbind) %do% {
  res <- exactprev(dp$seror[dp$antigenf==ab & dp$posmix==1])
  res <- data.frame(antigenf=ab,N=res[1],n=res[2],mean=res[3],min95=res[4],max95=res[5])
  res
}
```
...
knitr::kable(est_seroi,digits=3,
...
knitr::kable(est_seror,digits=3,

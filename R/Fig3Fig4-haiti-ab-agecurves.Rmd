...
subtitle: "Haiti age-dependent antibody curves, Figures 3 and 4"
output: 
  html_notebook:
    theme: default
    highlight: haddock
...
# Notebook Summary
This notebook estimates age-dependent mean antibody and seroprevalence curves in the Hatian cohort. Estimators include cubic splines (in a GAM model from the `mgcv` package) and  ensemble machine learning (using stacked super learning from the `tmleAb` package).  Spline-based estimates are reported in the main text in Figures 3 and 4. 
...
## Age-dependent seroprevalence curves, Figure 4
Create a more parsimonius plot, that only includes the spline fits but also includes the actual points.

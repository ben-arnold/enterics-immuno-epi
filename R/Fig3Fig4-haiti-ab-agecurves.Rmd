...
dplot <- dplot %>%
  mutate(antigenf=factor(antigenf,levels=c("Giardia VSP-3 + VSP-5","Cryptosporidium Cp17 + Cp23","Salmonella LPS groups B + D","ETEC toxin beta subunit","Norovirus GI","Norovirus GII","E. histolytica LecA")))

# bright color-blind palette defined in an earlier code chunk
pcols <- c(cred,corange,cgreen,cblue,cteal,cmagent)

ppcurve2 <- ggplot(data=dplot,aes(x=age,color=pathogen,fill=pathogen)) +
  # approximate simultaneous CI from spline fit

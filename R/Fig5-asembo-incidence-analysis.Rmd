...
rateplot <- ggplot(data=rate_plot_d,
                 aes(x=antigenf,y=rate,
                     ...
                     shape=outcome)) +
  geom_pointrange(aes(ymin=rate_lb,ymax=rate_ub,group=outcome),bg="white",
                  position=position_nudge(x=rep(c(-0.1,0.1),6) ) ) +
  labs(x="",y="Incidence rate per child-year\n") +
...
est_sero <- est_seroi %>%
  mutate(outcome="Seroconversion") %>%
  bind_rows(est_seror) %>%
  mutate(outcome=ifelse(is.na(outcome),"Seroreversion",outcome)) %>%
  arrange(antigenf,outcome)
...
seroip <- ggplot(data=cuminc_plot_d,
                 aes(x=antigenf,y=mean,
                     color=antigenf,fill=antigenf, 
                     shape=outcome)) +
  geom_pointrange(aes(ymin=min95,ymax=max95,group=outcome),bg="white",
                  position=position_nudge(x=rep(c(-0.1,0.1),6) ) ) +
  
  # facet_wrap(~factor(outcome),nrow=2,ncol=1)+
  labs(x="",y="Proportion seroconverted (6 month period)") +
...
```{r composite rate figure}
ratep <- grid.arrange(rateplot,seroip,ncol=1,nrow=2,heights=c(4,5))


# save pdf and a TIFF version
ggsave(file=here("figs","Fig5-asembo-seroincidence.pdf"),plot=ratep,device=cairo_pdf,height=8,width=4.5)
ggsave(file=here("figs","Fig5-asembo-seroincidence.TIFF"),plot=ratep,device="tiff",height=8,width=4.5)

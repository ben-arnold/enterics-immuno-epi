...
These estimates contribute to Figure 6 in the article, but they are first combined with cross-sectional estimators of the force of infection in the notebook `SI-File6-Fig6-asembo-cross-sectional-FOI-estimation.Rmd`
...
dsens_res <- foreach(ab=levels(dsens$pathogen),.combine=rbind) %:%
  foreach(deltafold=seq(1,10,by=0.1),.combine=rbind) %dopar% {
...
dsens_res2 <- dsens_res %>%
  mutate(antigenf = case_when(
    ...
    antigenf = factor(antigenf,levels=levels(dlp2$antigenf)),
...
```
## Figure of results
```{r foldchange sens figure,fig.height=8,fig.width=8}
# color palette
pcols <- cbPalette[c(2,6)]

# rate estimates for horizontal lines
scr_plot <- rate_ests %>% mutate(outcome="incident boosting") %>% filter(antigen!="cholera")
srr_plot <- rate_ests %>% mutate(outcome="incident waning") %>% filter(antigen!="cholera")
...
dsensp <- ggplot(data=dsens_plot,aes(x=foldchange,color=outcome,fill=outcome))+
  ...
  coord_cartesian(ylim=c(0,4),x=c(1,10))+
  theme_minimal() +
  theme(

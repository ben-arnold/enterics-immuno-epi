...
subtitle: Comparison of seroprevalence with seroconversion rates (force of infection) in Haiti and Kenya, Figure 6
output:
  html_notebook:
    highlight: haddock
    theme: default
...
pserop <- ggplot(data=d,aes(x=log(seroprev),y=log(seroi))) +
  ...
  labs(x="log seroprevalence (%)",y="force of infection\n(log seroconversion rate per child-year)")+
  theme_minimal() +
  theme(
...
pk <- ggplot(data=kd,aes(x=seroprev,y=seroi,color=antigenf,fill=antigenf)) +
  # geom_smooth(se=FALSE,color="gray50",size=0.5,method="loess",alpha=0.5,linetype="dashed")+
...
ph <- ggplot(data=hd,aes(x=seroprev,y=seroi,color=antigenf,fill=antigenf)) +
  # geom_smooth(se=FALSE,color="gray50",size=0.5,method="loess",alpha=0.5,linetype="dashed")+
  geom_errorbar(aes(ymin=seroi_lb,ymax=seroi_ub),width=0) +
  geom_point(size=2.5,pch=21,color="black") +
  scale_x_continuous(breaks=seq(0.6,1,by=0.1),labels=seq(60,100,by=10))+
  scale_y_continuous(breaks=seq(0,1.5,by=0.5))+
  scale_color_manual(values=pcols[c(1:5,7:8)],guide=FALSE) +
  scale_fill_manual(values=pcols[c(1:5,7:8)]) +
  guides(fill=guide_legend(title="Pathogen", 
                            reverse=TRUE,
                            keywidth=0.15,
                            keyheight=0.15,
...
ggsave(file=here("figs","Fig6-asembo-haiti-foi-vs-seroprev.pdf"),plot=ivsp_composite,device=cairo_pdf,height=8,width=6)
ggsave(file=here("figs","Fig6-asembo-haiti-foi-vs-seroprev.TIFF"),plot=ivsp_composite,device="tiff",height=8,width=6)

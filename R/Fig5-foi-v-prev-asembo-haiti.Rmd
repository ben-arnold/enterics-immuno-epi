...
subtitle: Comparison of seroprevalence with seroconversion rates (force of infection) in Haiti and Kenya, Figure 5
output:
  html_notebook:
    highlight: haddock
    theme: default
...
kd <- d %>% 
  filter(country=="Kenya" & method=="seroconversion definition:\ncrossed seropositivity cutoff") %>%
...
hd <- d %>% 
  filter(country=="Haiti" & method=="seroconversion definition:\ncrossed seropositivity cutoff") %>%
  mutate(antigeno = factor(antigenf,levels=c("E. histolytica LecA",
                                             "Giardia VSP-3 or VSP-5",
                                             "Norovirus GI.4",
                                             "Salmonella LPS groups B or D",
...
pcols <- cbPalette[c(4,2,5,3,1,8,6)]

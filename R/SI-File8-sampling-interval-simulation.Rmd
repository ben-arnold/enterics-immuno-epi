...
subtitle: Sampling interval simulation and sensitivity analyses in Haiti and Kenya
output: 
  html_document:
    highlight: haddock
    theme: default
...
IgG measurements in the Haitian cohort were separated by approximately 1 year, and in the Kenyan cohort by 6 months.  We hypothesized that since many enteric pathogens are thought to elicit transiently high IgG responses with rapid waning, tthis could lead us to miss seroconversion events in the interim periods. The empirical estimates in the primary analysis would thus under-estimate the true serological force of infection.  
...
# save the figures
# ggsave(here("figs", "haiti-interval-sim-example1.pdf"), plot = psim, device = cairo_pdf, width = 7, height = 5)
# ggsave(here("figs", "haiti-interval-sim-example2.pdf"), plot = psim_ds, device = cairo_pdf, width = 7, height = 5)
```
...
```{r haiti imputation simulation, eval=FALSE}
#-------------------------------
# simulate 100 datasets,
...
#-------------------------------
saveRDS(simdata, file = here("output", "haiti-imputation-simdata.rds"))
```
...
#-----------------------------
simdata <- readRDS(file = here("output", "haiti-imputation-simdata.rds"))
```
...
sim_res2 <- sim_res %>%
  ...
  # estimate the relative rate compared with a 30 day sampling interval
  group_by(antigenf,simnum) %>%
  mutate(
    seroi30 = max(ifelse(sinterval==30,seroi,-99)),
    seror30 = max(ifelse(sinterval==30,seror,-99)),
    ratio30i = seroi/seroi30,
    ratio30r = seror/seror30,
...
#-----------------------------
saveRDS(sim_res2,file=here("output","haiti-imputation-simulation-results.rds"))
...
# save a figure
# ggsave(filename = here("figs","haiti-interval-sim-rate-ests.pdf"), plot = simres_ratep,device = cairo_pdf, width = 3.5, height = 6)
...
# save a figure
# ggsave(filename = here("figs","haiti-interval-sim-rate-diff.pdf"), plot = simres_diffp,device = cairo_pdf, width = 3.5, height = 6)
...
## Figure: ratio of seroconversion rates

**Figure caption:** Simulation-based estimates of the fold-decrease in seroconversion rates (force of infection) in the Haitian cohort with longer sampling intervals. Estimates were generated through simulation of longitudinal IgG boosting and waning kinetics with daily resolution using boosting and decay parameters estimated from Haitian children ages 0-11 years old, which were then down-sampled at different frequencies. Points represent each of the 100 estimates from the simulated datasets and horizontal lines indicate median differences from the reference sampling interval of 30 days. Across pathogens, median differences in rates measured with a 360 day sampling interval were 50-70% lower than when estimated with a 30-day sampling interval.


```{r simulation figure rate ratio,fig.width=5,fig.height=8}
pcols <- cbPalette[c(2,3,4,6,8,8,7)]
sim_res_rr <- sim_res_plot 

simres_rrp <- ggplot(data=sim_res_rr,aes(x=factor(sinterval),y=ratio30i,color=antigenf)) +
  facet_wrap(~antigenf,ncol=2)+
  geom_jitter(width=0.15,alpha=0.3,pch=19,size=0.25)+
  stat_summary(fun.y = median, fun.ymin = median, fun.ymax = median,
                 geom = "crossbar", width = 0.5,color="black",size=0.2)+
  annotate("text",x=1,y=0.9,label="ref.",size=2,color="gray20")+
  scale_y_continuous(breaks=seq(1,0.2,by=-0.2),trans="log")+
  scale_color_manual(values=pcols)+
  coord_cartesian(ylim=c(0.2,1))+
  labs(y="ratio of seroconversion rates, 100 simulations",x="sampling interval, days") +
  theme_minimal()+
  theme(
    legend.position="none",
    strip.text=element_text(size=8)
...
# save a figure
# ggsave(filename = here("figs","haiti-interval-sim-rate-ratio.pdf"), plot = simres_rrp,device = cairo_pdf, width = 3.2, height = 6)

# print median ratios
sim_res_rr %>%
  group_by(antigenf,sinterval) %>%
  select(antigenf,sinterval,ratio30i,ratio30r) %>%
  summarize_all(median) %>%
  knitr::kable(
  digits = c(0,0,2,2),
  caption = "Median ratio in seroconversion and seroreversion rates per year at risk across 100 simulated datasets downsampled at different frequencies, with a 30-day sampling interval as the reference.",
  col.names = c("Antibody", "Sampling\ninterval (days)","Seroconversion\nrate","Seroreversion\nrate")
) %>%
  kable_styling(bootstrap_options = c("striped"), full_width = TRUE) 
```
...
```{r kenya imputation simulation, eval=FALSE}
#-------------------------------
# simulate 100 datasets,
# stored in a big data.frame
#-------------------------------
...
# it takes some time to generate
#-------------------------------
saveRDS(simdata, file = here("output", "asembo-imputation-simdata.rds"))
```
...
# antibodies for seroconversion
#-----------------------------
simdata <- readRDS(file = here("output", "asembo-imputation-simdata.rds"))
```
...
# save the simulation results
#-----------------------------
saveRDS(sim_res2,file=here("output","asembo-imputation-simulation-results.rds"))
...
# save a figure
# ggsave(filename = here("figs","asembo-interval-sim-rate-ests.pdf"), plot = simres_ratep,device = cairo_pdf, width = 3.4, height = 4.5)
...
The results of the simulations show thatseroconversion rates estimated with a 180 day sampling interval were internally consistent with the empirical estimates from the cohort. The simulations show that in cases of nearly continuous IgG boosting and waning, less frequenty measured cohorts will underestimate force of infection, but the magnitude of difference was < 1 episode per year for all pathogens except for ETEC and _Campylobacter_ (below).
...
# save a figure
# ggsave(filename = here("figs","asembo-interval-sim-rate-diff.pdf"), plot = simres_diffp,device = cairo_pdf, width = 3.5, height = 4.5)
...
**Figure caption:** Simulation-based estimates of the fold-decrease in seroconversion rates (force of infection) in the Asembo cohort with longer sampling intervals. Estimates were generated through simulation of longitudinal IgG boosting and waning kinetics with daily resolution using boosting and decay parameters estimated from Kenyan children ages 4-17 months old, which were then down-sampled at different frequencies. Points represent each of the 100 estimates from the simulated datasets and horizontal lines indicate median differences from the reference sampling interval of 30 days. Across pathogens, median rates measured with a 180 day sampling interval were 20-80% lower than when estimated with a 30-day sampling interval. Note that the large relative decline in rates for _E. histolytica_ and _Salmonella_ were in part due to very low seroconversion rates overall for those pathogens in this cohort, and do not represent large differences in rates (previous figures). 

```{r kenya simulation figure rate ratio ,fig.width=5,fig.height=8}
...
# save a figure
# ggsave(filename = here("figs","asembo-interval-sim-rate-ratio.pdf"), plot = simres_rrp,device = cairo_pdf, width = 3.2, height = 4.5)

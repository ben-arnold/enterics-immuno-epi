...
pcols <- c(cred,corange,cgreen,cteal,cblue,cgrey)
...
# use ROC-based cutoffs if 
# available, or mixture-model
# based cutoffs if necessary
#-----------------------------
...
# re-arrange the antibody factor to group pathogens on the same row
pgamfits <- pgamfits %>%
  mutate(antigenf = factor(antigenf,levels=levels(antigenf)[c(1:4,6:9,5)]))

# bright color-blind palette defined in an earlier code chunk
# exclude teal because ETEC does not have seropositive cutoff
pcols <- c(cred,corange,cgreen,cblue,cgrey)
...
# bright color-blind palette defined in an earlier code chunk
# exclude cteal because ETEC doesn't have a seropositive cutoff
pcols <- c(cred,corange,cgreen,cblue,cgrey)

ppcurve2 <- ggplot(data=pgamfits,aes(x=age,group=pathogen,color=pathogen,fill=pathogen)) +
  # approximate simultaneous CI from spline fit
  geom_ribbon(aes(ymin=lwrS,ymax=uprS),alpha=0.2,color=NA,fill=cgrey) +
  # actual measurements
  geom_jitter(aes(y=posmix),width=0.2,height=0,color="black",alpha=0.4,size=0.3) +

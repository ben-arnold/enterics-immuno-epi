...
#-----------------------------
dl <- readRDS("~/enterics-seroepi/data/kongwa_analysis2.rds")
...
# save a PDF version 
ggsave("~/enterics-seroepi/figs/kongwa-ab-agecurves-mean-allestimates.pdf",plot=pcurveall,device="pdf",width=7,height=14)
...
pcurve <- ggplot(data=gamfits,aes(x=age+0.5,color=pathogen,fill=pathogen)) +
  # approximate simultaneous CI from spline fit
  geom_ribbon(aes(ymin=lwrS,ymax=uprS),alpha=0.2,color=NA,fill=cgrey) +
  # actual measurements
...
# save a PDF version 
ggsave("~/enterics-seroepi/figs/kongwa-ab-agecurves-mean.pdf",plot=pcurve,device="pdf",width=7,height=14)
...
# for the fits
library(mgcv)
pgamfits <- foreach(ab=c("Giardia VSP-3 + VSP-5","Cryptosporidium Cp17 + Cp23","E. histolytica LecA"),.combine=rbind) %dopar% {
...
set.seed(12345)
pslfits <-  foreach(ab=c("Giardia VSP-3 + VSP-5","Cryptosporidium Cp17 + Cp23","E. histolytica LecA"),.combine=rbind) %dopar% {
  pd <- filter(dlp, antigenf==ab)
...
tmlemeansp <- foreach(ab=c("Giardia VSP-3 + VSP-5","Cryptosporidium Cp17 + Cp23","E. histolytica LecA"),.combine=rbind) %:%
  foreach(agei=1:9,.combine=rbind) %dopar% {
...
# save a PDF version
ggsave("~/enterics-seroepi/figs/kongwa-ab-agecurves-seroprev-allests.pdf",plot=ppcurve,device="pdf",width=4,height=10)
...
# save a PDF version
ggsave("~/enterics-seroepi/figs/kongwa-ab-agecurves-seroprev.pdf",plot=ppcurve2,device="pdf",width=4,height=10)
...
saveRDS(gamfits,file="~/enterics-seroepi/output/kongwa-enteric-ab-gamfits-means.rds")
# ensemble fits of geometric means
saveRDS(slfits,file="~/enterics-seroepi/output/kongwa-enteric-ab-slfits-means.rds")
# spline fits of age-dependent seroprevalence
saveRDS(pgamfits,file="~/enterics-seroepi/output/kongwa-enteric-ab-gamfits-seroprev.rds")
# spline fits of age-dependent seroprevalence
saveRDS(pslfits,file="~/enterics-seroepi/output/kongwa-enteric-ab-slfits-seroprev.rds")
```

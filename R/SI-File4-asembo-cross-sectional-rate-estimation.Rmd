---
title: Enteropathogen seroepidemiology among children in high transmission settings
subtitle: Supplementary Information File 4. Estimation of seroconversion rates assuming cross-sectional ("current status") data in Kenya.
output:
  html_document:
    highlight: haddock
    theme: default
...
For enteric antibodies with defined cutoffs for seropositivity and approximately monotonically increasing age-dependent seroprevalence, estimate the hazard of seroconversion, or seroconversion rate, which is one measure of the force of infection.
...
#--------------------------------
# restrict the data to antigens
# with increasing seroprevalence
# exclude rotavirus (decreasing)
#--------------------------------
dfoi <- d %>%
  filter(!antigen %in% c("rotavirus")) %>%
...
knitr::kable(glmfoi,digits=3,
             caption="Average incidence rate per child month estimated from a cross-sectional seroprevalence curve, exponential (constant rate) model",
             col.names = c("Antigen","Incidence per month","min95","max95"),
             row.names = FALSE)
...
est_r <- readRDS("~/enterics-seroepi/output/asembo-enteric-ab-ests-incidencerates.rds") %>%
  filter(outcome=="Seroreversion" & !antigenf %in% "Rotavirus") %>%
  mutate(antigenf=droplevels(antigenf)) %>%
...
knitr::kable(rcmests,digits=3,
             caption="Average incidence rate per child month estimated from a cross-sectional seroprevalence curve, reversible catalytic model",
             col.names = c("Antigen","Sero-reversion rate","Incidence per month","min95","max95"),
             row.names = FALSE)
...
pfoi <- ggplot(data=gamfoi,aes(x=age,fill=antigenf,color=antigenf)) +
  ...
  labs(x="Age in months",y="Age-specific seroconversion rate (incidence per child-month)") +
...
# save a PDF version
ggsave("~/enterics-seroepi/figs/asembo-ab-agecurves-foi.pdf",plot=pfoi,device="pdf",width=5,height=7)
```
...
plabs <- gamfoi %>%
  ...
  mutate(foi=ifelse(antigenf=="E. histolytica LecA",foi-0.002,foi),
         foi=ifelse(antigenf=="Salmonella LPS groups B + D",foi+0.002,foi))
...
pfoi2 <- ggplot(data=gamfoi,aes(x=age,group=antigenf,color=antigenf,fill=antigenf)) +
  ...
  labs(x="Age in months",y="Seroconversion rate") +
  scale_fill_manual(values=pcols) +
  scale_color_manual(values=pcols) +
...
Compact figure with all pathogens on the same panel and nicer labels
```{r foi figure small with labels}
# bright color-blind palette defined in an earlier code chunk
pcols <- c(cred,corange,cgreen,cteal,cblue,cmagent,cgrey)
# pcols <- c(corange,cred,cmagent,cblue,cteal,cgreen,cgrey)
...
pfoi2 <- ggplot(data=gamfoi,aes(x=age,group=antigenf,color=antigenf,fill=antigenf)) +
  ...
  geom_hline(aes(yintercept=0)) +
  # pathogen labels
  geom_text(data=plabs,aes(x=age+0.5,y=foi,label=antigenf),
            hjust=0)+
  # plot aesthetics
  scale_x_continuous(breaks=seq(4,16,by=2))+
  # scale_y_continuous(breaks=seq(0,1,by=0.2),labels=sprintf("%1.0f",seq(0,1,by=0.2)*100))+
...
pfoi3 <- ggplot_gtable(ggplot_build(pfoi2))
pfoi3$layout$clip[pfoi3$layout$name == "panel"] <- "off"
grid.draw(pfoi3)
# pdf(file="~/enterics-seroepi/figs/asembo-ab-agecurves-foi-small.pdf",width=9,height=8)
# grid.draw(pfoi3)
# dev.off()
...
knitr::kable(select(avgfoi,-mufoi_se),digits=3,
             caption="Average incidence rate per child month estimated from a cross-sectional seroprevalence curve",
             col.names = c("Antigen","Incidence per month","min95","max95"),
             row.names = FALSE)

```

Indeed the estimates are very similar with those above, averaged over the empirical distribution of age.  Given this similarity, the population-averaged estimates will be used below in the comparison across methods.
...
ratecomp_plot <- ggplot(data=rate_comp,aes(x=antigenf,y=rate,color=antigenf,fill=method,shape=method)) +
  geom_pointrange(aes(ymin=rate_lb,ymax=rate_ub),
                  fill="white",size=0.5,
                  position=position_nudge(x=c(rep(c(-0.3,-0.1,0.1,0.3),3),
                                              0,
                                              rep(c(-0.3,-0.1,0.1,0.3),4)))) +
  
  labs(x="",y="seroconversion rate per child-year") +
  ...
  theme_minimal() +
  theme(legend.position = "right",
        axis.text.x=element_text(size=8,angle=45,hjust = 1,color=pcols),
        strip.text = element_text(size=12),)

ratecomp_plot

# save a PDF version
ggsave("~/enterics-seroepi/figs/asembo-ab-seroincidence-rate-comparison.pdf",plot=ratecomp_plot,device="pdf",width=8.5,height=4)

```

For most pathogens, the semiparametric estimate of seroconversion rates using splines was close to the rate estimated prospectively. In all cases uncertainty in spline-based estimates of the seroconversion rate derived from seroprevalence was much higher than the prospective rate estimates, as reflected by wider 95% credible intervals.   As Jewell and Emerson (**2013**) demonstrate under a parametric exponential survival model, the asymptotic relative efficiency of a cross-sectional, current-status analysis compared to a longitudinal analysis is 96.1% if the cumulative proportion of failures is 0.5, but only 65.5% if the proportion of failures is 0.9. These results are broadly consistent with that analysis, with the loss in efficiency much higher among ETEC, _V. cholerae_ and _Campylobacter_ with seroprevalence that approached 100% by ages 10-12 months in this cohort.

For most pathogens, seroconversion rate estimates from cross-sectional analysis were biased downward compared to the longitudinal analysis -- a result that would be expected in the case of seroreversion, which was common in this cohort for all but ETEC, _V. cholerae_, and _Campylobacter_.  The parametric cross-sectional analyses assumed a constant rate without seroreversion (exponential survival model, SIR model) or with seroreversion (reversible catalytic model, SIS model), and were genearlly further from the longitudinal estimates than the semiparametric spline; exceptions include _E. histolytica_ and _Salmonella_. Age-specific estimates of the seroconversion rate under the spline model suggest that the rates were not constant over age, so this suggests the parametric models could be slightly more biased. Allowing for sero-reversion in the reversible catalytic model did not remove the bias.

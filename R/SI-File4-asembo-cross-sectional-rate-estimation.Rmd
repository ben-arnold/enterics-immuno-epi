...
subtitle: Supplementary Information File 4. Estimation of seroconversion rates assuming cross-sectional ("current status") data in Kenya. Figure 6 - supplement 1
output:
  html_document:
    highlight: haddock
    theme: default
...
pfoi2 <- ggplot(data=gamfoi,aes(x=age,group=antigenf,color=antigenf,fill=antigenf)) +
  ...
  labs(x="Age in months",y="Seroconversion rate per child-month") +
  scale_fill_manual(values=pcols) +
...
est_long <- readRDS("~/enterics-seroepi/output/asembo-enteric-ab-ests-incidencerates.rds") %>%
  filter(outcome=="Seroconversion" & antigen!="cholera") %>%
...
```
#Figure 6 - supplement 1, Rate Comparison
...
ggsave(here("figs","Fig6sup1-asembo-ab-seroincidence-rate-comparison.pdf"),plot=ratecomp_plot,device="pdf",width=7.5,height=4)
ggsave(here("figs","Fig6sup1-asembo-ab-seroincidence-rate-comparison.TIFF"),plot=ratecomp_plot,device="tiff",width=7.5,height=4)
...
rate_comp2 <- rate_comp %>%
  mutate(pathgroup=ifelse(antigenf %in% c("ETEC toxin beta subunit","Campylobacter p18 or p39"),"Higher transmission","Lower transmission"),
         pathgroup=factor(pathgroup,levels=c("Lower transmission","Higher transmission")))


ratecomp_plot2 <- ggplot(data=rate_comp2,aes(color=antigenf,fill=method,shape=method)) +
...
# save PDF and TIFF versions
# ggsave(here("figs","Fig6sup1-asembo-ab-seroincidence-rate-comparison.pdf"),plot=ratecomp_plot2,device="pdf",width=8,height=4)
# ggsave(here("figs","Fig6sup1-asembo-ab-seroincidence-rate-comparison.TIFF"),plot=ratecomp_plot2,device="tiff",width=8,height=4)

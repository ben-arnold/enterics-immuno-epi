...
library(mgcv)
pgamfits <- foreach(tri=levels(dlp$tr),.combine=rbind) %:%
  foreach(ab=levels(dlp$antigenf),.combine=rbind) %dopar% {
    pd <- filter(dlp, tr==tri & antigenf==ab)
    gfit <- mgcv::gam(seropos~s(age, bs="cr")+s(childid,bs="re",by=dummy), data=pd,family="binomial")
    newd <- pd %>% mutate(dummy=0)
    gsci <- gamCI(m=gfit,newdata=newd,nreps=1000)
    gsci <- gsci %>% mutate(tr=tri,antigenf=ab)
    return(gsci)
}
...
# seroprevalence among control
# knitr::kable(filter(tmlemeans,tr=="Cont"),digits=2,caption="Seroprevalence in the control arm")
...
# for the fits
#-----------------------------
gamfits <- foreach(tri=levels(dl$tr),.combine=rbind) %:%
  foreach(ab=levels(dl$antigenf),.combine=rbind) %dopar% {
    pd <- filter(dl, tr==tri & antigenf==ab) %>%
      mutate(dummy=1)
    gfit <- mgcv::gam(logmfi~s(age, bs="cr",k=9)+s(clusterid,bs="re",by=dummy) ,data=pd,family="gaussian")
    newd <- pd %>% mutate(dummy=0)
    gsci <- gamCI(m=gfit,newdata=newd,nreps=1000)
    gsci <- gsci %>% mutate(tr=tri,antigenf=ab)
    return(gsci)
...
# seroprevalence among intervention and control
knitr::kable(arrange(ktmlemeans,antigenf,tr),digits=2,caption="Seroprevalence in the Intervention and Control groups")  %>%
  kable_styling(bootstrap_options = "striped",full_width = TRUE)

# difference between groups
knitr::kable(ktmlediffs,digits=2,caption="Difference in seroprevalence (Intervention minus Control)")  %>%
  kable_styling(bootstrap_options = "striped",full_width = TRUE)

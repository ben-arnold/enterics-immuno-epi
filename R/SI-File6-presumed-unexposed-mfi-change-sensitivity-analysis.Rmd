...
subtitle: Supplementary Information File 6. Sensitivity analysis of change in IgG used to identify presumed unexposed measurements in Haiti and Kenya.
output: 
  html_notebook:
    theme: default
    highlight: haddock
...
gridmix <- foreach(ab=levels(dmix$antigenf),.combine=rbind) %:%
  foreach(icut=seq(0.3,2.5,by=0.01),.combine=rbind) %dopar% {
    ...
    if(nrow(dmu)>=2) {
      ...
      tabi <- data.frame(mix0cuti0=NA,mix1cuti0=NA,mix0cuti1=NA,mix1cuti1=NA,Nobs=NA,discordance=NA,agreement=NA,kappa=NA,antigenf=ab,abchange=icut,abcutoff=NA)
...
gridroc <- foreach(ab=levels(droc$antigenf),.combine=rbind) %:%
  foreach(icut=seq(0.3,2.5,by=0.01),.combine=rbind) %dopar% {
    ...
    tabi <- tabi %>%
      mutate(Nobs=roc0cuti0+roc1cuti0+roc0cuti1+roc1cuti1,
             discordance=(roc1cuti0+roc0cuti1)/Nobs,
             agreement=(roc0cuti0+roc1cuti1)/Nobs,
             kappa=kappai,
             antigenf=ab,
             abchange=icut,
...
gridmix <- foreach(ab=levels(dmix$antigenf),.combine=rbind) %:%
  foreach(icut=seq(0.3,2.5,by=0.01),.combine=rbind) %dopar% {
    ...
    if(nrow(dmu)>=2) {
      ...
      tabi <- tabi %>%
        mutate(Nobs=mix0cuti0+mix1cuti0+mix0cuti1+mix1cuti1,
             discordance=(mix1cuti0+mix0cuti1)/Nobs,
             agreement=(mix0cuti0+mix1cuti1)/Nobs,
             kappa=kappai,
             antigenf=ab,
             abchange=icut,

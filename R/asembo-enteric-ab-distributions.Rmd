...
mbavars <- c("vsp3","vsp5","cp17","cp23","leca","rotavirus","etec","cholera","salb","sald","p18","p39")

mbalabs <- c("Giardia VSP-3","Giardia VSP-5","Cryptosporidium Cp17","Cryptosporidium Cp23","E. histolytica LecA","Rotavirus","ETEC toxin beta subunit","Cholera toxin beta subunit","Salmonella LPS B","Salmonella LPS D","Campylobacter p18","Campylobacter p39")
```
...
# save a PDF version
ggsave("~/enterics-immuno-epi/figs/asembo-ab-distributions.pdf",plot=p,device="pdf",width=7,height=14)
...
# save a PDF version
ggsave("~/enterics-immuno-epi/figs/asembo-ab-mixture-distributions.pdf",plot=pmix,device="pdf",width=7,height=14)

```

## Cutoffs derived from presumed unexposed
Children whose antibody levels increased by at least 2 on the log10 scale were presumably unexposed at their first measurement. Use them to identify an unexposed distribution.
...
#-----------------------------
pcols <- c(cred,corange,cteal,cteal,cgrey)
...
# save a PDF version
ggsave("~/enterics-immuno-epi/figs/asembo-ab-unexposed-distributions.pdf",plot=punexp,device="pdf",width=7,height=14)

```


## Figure of mixture and unexposed distributions together
...
pall <- ggplot(data=dl,aes(x=logmfi,group=pathogen,color=pathogen,fill=pathogen)) +
  ...
  # plot empirical smoothed distribution
  geom_density(aes(y=..density..),color=NA,alpha=0.7)+
  # plot empirical distribution among the unexposed
  # geom_density(data=filter(dl,unexp=="Presumed unexposed" & !is.na(unexpcut)),
  #              aes(y=..density..),color=cgrey,fill=NA) +
  # geom_histogram(data=filter(dl,unexp=="Presumed unexposed" & !is.na(unexpcut)),
  #              aes(y=..density..),color=cgrey,fill=NA) +
  # plot fitted mixture distributions
...
# save a PDF version
ggsave("~/enterics-immuno-epi/figs/asembo-ab-mixunexp-distributions.pdf",plot=pall,device="pdf",width=7,height=14)

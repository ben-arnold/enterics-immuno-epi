...
subtitle: "Supporting Information File 6: Estimation of age-dependent means and seroprevalence using multiple approaches, Figure 3"
output: 
  html_notebook:
    theme: default
    highlight: haddock
...
pcurve <- ggplot(data=gamfits,aes(x=age,color=pathogeng,fill=pathogeng)) +
  # approximate simultaneous CI from spline fit
  geom_ribbon(aes(ymin=lwrS,ymax=uprS),alpha=0.4,color=NA) +
  # actual measurements
  geom_jitter(aes(y=logmfi),width=0.2,height=0,alpha=0.4,size=0.3,color=cgrey) +
  # spline fit
  geom_line(aes(y=fit)) +
  # labels
...
ppcurve2 <- ggplot(data=dplot,aes(x=age,color=antigenf,fill=antigenf)) +
  ...
  geom_ribbon(aes(ymin=lwrS,ymax=uprS),alpha=0.4,color=NA) +
  # actual measurements
  geom_point(aes(y=seropos),alpha=0.5,size=1.5,pch="|",color=cgrey) +
  # spline fit
  geom_line(aes(y=fit)) +
  # labels
...
ppcurve2 <- ggplot(data=dplot,aes(x=age,color=antigenf,fill=antigenf)) +
  ...
  geom_ribbon(aes(ymin=lwrS,ymax=uprS),alpha=0.4,color=NA) +
  # actual measurements
  geom_jitter(aes(y=seropos),width=0.2,height=0,alpha=0.5,size=1.5,pch="|",color=cgrey) +
  # spline fit
  geom_line(aes(y=fit)) +
  # labels
...
pcurve <- ggplot(data=dplot,aes(x=age+0.5,color=pathogen,fill=pathogen)) +
  ...
  geom_ribbon(aes(ymin=lwrS,ymax=uprS),alpha=0.4,color=NA) +
  # actual measurements
  geom_jitter(aes(y=logmfi),width=0.2,height=0,alpha=0.2,size=0.2,color=cgrey) +
  # spline fit
  geom_line(aes(y=fit)) +
  # labels
  geom_text(data=plabs,aes(x=0,y=4.55,label=antigenf,hjust=0,vjust=0),color="gray20")+
...
ppcurve2 <- ggplot(data=dplot2,aes(x=age+0.5,color=antigenf,fill=antigenf)) +
  ...
  # actual measurements (do not plot -- not informative b/c too many points)
  # geom_jitter(aes(y=seropos),width=0.2,height=0,color="black",alpha=0.4,size=0.3) +
  # spline fit
  geom_line(aes(y=fit)) +
  # labels
